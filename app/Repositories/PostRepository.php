...
class PostRepository
{
            ...
            if ($doImage) {
                ...
                if ($matches) {
                    if (isset($matches[1]) and $matches[1] and preg_match('#http://|https://#', $matches[1])) {
                        if ($this->checkRemoteFile($matches[1])) {
                            $images[] = $matches[1];
                        }

                    } else {
                        if (preg_match('#http://|https://#', $matches[2]) and $this->checkRemoteFile($matches[2])) {
                            $images[] = $matches[2];
                        }
                ...
                for ($i = 0; $i < $imgElements->length; $i++) {
                    ...
                    if (count($images) <= 5 and preg_match("#http:\/\/|https:\/\/#", $cImg->getAttribute('src'))) {
                        if ($this->checkRemoteFile($cImg->getAttribute('src'))) {
    ...
    public function add($val, $user = null)
    {
        ...
        $post->text           = ($text); //\Hook::fire('filter-text', sanitizeText($text));
        $post->user_id        = $user->id;
        $post->to_user_id     = ($to_userid == $user->id) ? 0 : $to_userid ? $to_userid : 0;
        ...
        $post->community_id   = $community_id ? $community_id : 0;
        $post->page_id        = $page_id ? $page_id : 0;
    ...
    public function delete($id)
    {
        ...
        if ($post->video_path != '') {
            ...
            if ($CDNRepository->has($post->video_path)) {
                $CDNRepository->delete($post->video_path);
            }
        ...
        if ($post->file_path != '') {
            ...
            if ($CDNRepository->has($post->file_path)) {
                $CDNRepository->delete($post->file_path);
            }
